...
def merge_edge(node):
    ...
    # Sort by the degrees of the neighbors and select the top two with the highest degrees
    sorted_neighbors = sorted(neighbors, key=lambda x: BA_graph.degree[x], reverse=True)
...
while target_edges < BA_graph.number_of_edges():
    print(f'edges: {BA_graph.number_of_edges()}')
...
result_df.to_csv(f'output\PCN_{f}_MaxOut_multiplied_matrix.csv')
